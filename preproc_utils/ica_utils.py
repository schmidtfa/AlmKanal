...
def run_ica(raw,
    ...
    if len(bads) > 0:
        bad_ids = np.concatenate(bads).astype(int).tolist()
    else:
        bad_ids = []
    ...
    if img_path != None:
        if len(bad_ids) > 0:
            ...
            fig = ica.plot_components(picks=bad_ids, 
                                    ch_type='mag', inst=raw, title=titles, 
                                    show=False)
            plot_file_name = f"{fname}_ICA_summary_plot.png"
            # if the directory doesn't exist, create it
            if not os.path.exists(os.path.join(img_path, 'ica_pngs')):
                os.makedirs(os.path.join(img_path, 'ica_pngs'))
            
            fig.savefig(os.path.join(img_path, 'ica_pngs/' , plot_file_name))
        else:
...
def find_train_ica(raw, 
    ...
    train_peaks = get_band_info(irasa_out.get_peaks(), 
                                freq_range=(train_freq-1, train_freq+1), 
                                ch_names=np.arange(ic_signal.shape[0])).dropna()
