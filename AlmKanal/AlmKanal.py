...
from src_utils.src_utils import data2source, src2parc
from data_utils.check_data import check_raw_epoch
from data_utils.data_classes import info_class, pick_dict_class
...
class AlmKanal:
    ...
    pick_dict: dict = pick_dict_class(meg=True, eog=True, ecg=True, eeg=False, stim=True)
    ...
    info: dict = field(default=attrs.Factory(info_class))
        ...
        if self.fwd is not None:
            ...
            if return_parc:
                if np.logical_and(subject_id is not None, subjects_dir is not None):
                    ...
                    raise ValueError('You need to set the correct name for the `subject_id` and `subjects_dir` if you want to parcels.')
